...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id, $category_id = NULL, $filter_order_code = NULL) {
		$sql = "
			...
			(SELECT position FROM " . DB_PREFIX . "spare_part sp WHERE sp.product_id = p.product_id AND sp.category_id = '" .(int)$category_id. "' LIMIT 1) AS position,
		...
		if (!empty($filter_order_code)) {
			$sql .= "AND p.order_code LIKE '%" . $filter_order_code . "%'";
		}
		...
		if ($query->num_rows) {
			return array(
				...
				'isbn'             					=> $query->row['isbn'],
				'mpn'              					=> $query->row['mpn'],
				'on_demand'									=> $query->row['on_demand'],
				'location'         					=> $query->row['location'],
				'quantity'         					=> $query->row['quantity'],
				'highlights'       					=> $query->row['highlights'],
				'techspecs'        					=> $query->row['techspecs'],
				'custom_field_1_title' 			=> $query->row['custom_field_1_title'],
				'custom_field_2_title' 			=> $query->row['custom_field_2_title'],
				'custom_field_3_title' 			=> $query->row['custom_field_3_title'],
				'custom_field_1_content'		=> $query->row['custom_field_1_content'],
				'custom_field_2_content'		=> $query->row['custom_field_2_content'],
				'custom_field_3_content'		=> $query->row['custom_field_3_content'],
				'breakup_id'       					=> $query->row['position'],
				'external_url_id'  					=> $query->row['external_url_id'],
				...
				'order_code'			 					=> $query->row['order_code'],
	...
	public function getProducts($data = array()) {
       ...
       ( 
		...
		if (!empty($data['filter_order_code'])) {
			$filter_order_code = $data['filter_order_code'];
			$sql .= " AND p.order_code LIKE '%" . $data['filter_order_code'] . "%'";
		...
		if (!empty($data['filter_name']) || !empty($data['filter_tag'])) {
			...
			if (!empty($data['filter_name'])) {
				...
				foreach ($words as $word) {
					$implode[] = "pd.name LIKE '%" . $this->db->escape($word) . "%' OR p.order_code LIKE '%" . $this->db->escape($word) . "%'";
				}
		...
		foreach ($query->rows as $result) {
			$product_data[$result['product_id']] = $this->getProduct($result['product_id'], $data['filter_category_id'], $filter_order_code);
		}

		return $product_data;
	}

	public function getSpareParts($data = array()) {
		$sql = "
			SELECT			sp.product_id, 
									sp.position AS breakup_id, 
									sp.category_id, 
									p.on_demand, 
									p.order_code,
									pd.name,
									pd.language_id,
									p.external_url_id
			FROM 				" . DB_PREFIX . "spare_part sp	
			LEFT JOIN 	" . DB_PREFIX . "product p
			ON 					(sp.product_id = p.product_id)	
			LEFT JOIN		" . DB_PREFIX . "product_description pd
			ON					(sp.product_id = pd.product_id)
			WHERE 			sp.category_id = '" . $data['filter_category_id'] . "' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "'
		";
	...
	public function getProductDownloads($product_id) {
		$query = $this->db->query("
			SELECT DISTINCT p2d.download_id, d.filename, dd.name
